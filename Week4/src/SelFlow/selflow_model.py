...
class SelFlowModel(object):
    ...
    def test(self, restore_model, save_dir, is_normalize_img=True):
        ...
        for i in range(dataset.data_num):
            ...
            plt.imsave('%s/flow_fw_color_%s.png' % (save_dir, save_name_list[i]), np_flow_fw_color[0])
            plt.imsave('%s/flow_bw_color_%s.png' % (save_dir, save_name_list[i]), np_flow_bw_color[0])
