...
def dump_imgs(output_path, track):
    """Dump images from img_paths to output_path/id"""
    output_path = os.path.join(output_path, '%06d' % track.id)
...
def clustering(args, tracks):
    ...
    if args.cluster == 'kmeans':
        ...
        Cluster = MiniBatchKMeans(n_clusters=args.k, max_iter=500, n_init=3, init_size=args.k, verbose=1,
                                  max_no_improvement=250, batch_size=1000)
...
def multi_camera_matching(opt, MCT, loc_seq_id):
    ...
    if opt.method == 'cluster':
        ...
        with open(os.path.join(opt.output_path, 'cluster.pkl'), 'wb') as f:
        ...
        for i, loc_tracks in enumerate(MCT):
            ...
            with open(os.path.join(opt.output_path, 'Loc%d_cluster.pkl' % (i + 1)), 'wb') as f:
                pickle.dump(classes, f)
            # with open(os.path.join(opt.output_path, 'Loc%d_cluster.pkl' % (i+1)), 'rb') as f:
        ...
        with open(os.path.join(opt.output_path, 'all_loc_cluster.pkl'), 'wb') as f:
            pickle.dump(classes, f)
        # with open(os.path.join(opt.output_path, 'all_loc_cluster.pkl'), 'rb') as f:
        #    classes = pickle.load(f)
        ...
        for i, loc_tracks in enumerate(MCT):
            ...
            for class_id in tqdm(np.unique(classes).tolist()):
                ...
                assert debug_frame(track.dump())
                Locs[i].append(track)
        with open(os.path.join(opt.output_path, 'Locs.pkl'), 'wb') as f:
            pickle.dump(Locs, f)
        # with open(os.path.join(opt.output_path,'Locs.pkl'),'rb') as f:
        #    Locs  = pickle.load(f)
        ###############

        ###############

        q_idx = list(range(n_seq))
        if opt.method == 're-rank-all':
            ...
            q_idx = list(range(n_seq))
            multi_loc_tracks = []
            iterator = list(range(n_seq))
            iterator.pop(-1)

            for i in iterator:
                tracks = Locs[n_seq - 1] + Locs[i]
                dis_matrix = compute_distance(tracks)
                rank_list = re_ranking(dis_matrix, q_idx)
                with open(os.path.join(opt.output_path, 're_rank%d.pkl' % i), 'wb') as f:
                    pickle.dump(rank_list, f)
                # with open(os.path.join(opt.output_path, 're_rank%d.pkl' % i), 'rb') as f:
            ...
            with open(os.path.join(opt.output_path, 'after_re_ranking.pkl'), 'wb') as f:
                pickle.dump(multi_loc_tracks, f)

            # with open(os.path.join(opt.output_path, 'after_re_ranking.pkl'), 'rb') as f:
        ...
        # with open(os.path.join(opt.output_path, 'ref_tracks.pkl'), 'wb') as f:
        #    pickle.dump(ref_tracks, f)
        # with open(os.path.join(opt.output_path, 'tgt_tracks.pkl'), 'wb') as f:
        #    pickle.dump(tgt_tracks, f)
        # with open(os.path.join(opt.output_path, 'ref_tracks.pkl'), 'rb') as f:
        #    ref_tracks = pickle.load(f)
        # with open(os.path.join(opt.output_path, 'tgt_tracks.pkl'), 'rb') as f:
        ...
        '''
            ...
            #with open(os.path.join(opt.output_path, 'knn_graph.pkl'), 'wb') as f:
            #    pickle.dump(knn_graph, f)
            hac = AgglomerativeClustering(linkage='ward',
                                          connectivity=None,
                                          n_clusters=opt.k)
            classes = hac.fit_predict(features)
            with open(os.path.join(opt.output_path, 'hac_classes.pkl'), 'wb') as f:
                pickle.dump(classes, f)
...
if __name__ == '__main__':
    ...
    # with open(os.path.join(args.output_path, 'after_mct.pkl'), 'rb') as f:
    #    tracks = pickle.load(f)
    # print(len(tracks))
    # sys.exit()
