...
def extract_features(tracks, temp_dir, reid_model, n_layers, batch_size):
    ...
    img_paths = np.array([str(pathlib.Path(os.path.join(temp_dir, img)).resolve()) for img in img_names])
    reid_model = ResNet_Loader(reid_model, n_layers, batch_size)

    indices = []
    for tr in tracks:
        indices.append(int(tr.id))
    img_names = np.array(img_names)
    img_names = img_names[indices].tolist()
    img_paths = img_paths[indices]

    features = reid_model.inference(img_paths.tolist()).numpy()

    # Import features to tracks
    # (Detections should already be sorted by id)
    img_ids = np.array([int(n.split('.')[0]) for n in img_names])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--size_th', default=70, type=int,
                        help='filter tracks that do not have detection larger than "filter size" in it longest edge')
